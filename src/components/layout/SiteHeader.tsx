            ...
            {navItems.map((item) => {
              ...
              return (
                <div key={item.to} className="group relative">
                  ...
                  {item.children && (
                    <div className="invisible absolute left-0 top-full -mt-2 z-[1300] min-w-[220px] rounded-none border-x border-b border-border border-t-[3px] border-t-primary bg-popover py-1 opacity-0 shadow-[var(--shadow-elegant)] transition-all duration-150 group-hover:visible group-hover:opacity-100">
                      {item.children.map((c) => (
                        <Link
                          key={c.to}
                          to={c.to}
