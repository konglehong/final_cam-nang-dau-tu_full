            ...
            {navItems.map((item) => {
              ...
              return (
                <div key={item.to} className="group relative">
                  ...
                  {item.children && (
                    <div className="invisible absolute left-0 top-full z-[1300] -mt-1 min-w-[200px] translate-y-1 rounded-xl border border-border bg-popover py-1 opacity-0 shadow-[var(--shadow-elegant)] transition-all duration-150 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">
                      {item.children.map((c) => (
                        <Link
                          key={c.to}
                          to={c.to}
